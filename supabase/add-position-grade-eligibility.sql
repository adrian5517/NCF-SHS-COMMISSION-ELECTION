-- Run this in the Supabase SQL editor for project ynuxlofntavdeyhtjkof to add
-- per-position grade-level eligibility (only certain grade levels may vote
-- in a position). Safe to re-run: the ALTER is idempotent and the functions
-- below are create-or-replace.
...
-- get_ballot: hide positions the student's grade can't vote in. An empty
-- eligible_grade_levels ('{}') means everyone can vote in that position.
...
-- submit_ballot: reject any position the student's grade isn't allowed to
-- vote in, and only record votes/abstentions for eligible positions.
